  ...
  <div class="container">
    <div class="d-flex justify-content-between align-items-center py-2">
      <h1 class="judul-header pt-4">Produk Terbaru</h1>
    </div>
        ...
        <div class="card-body">
          <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
              ...
              <li class="breadcrumb-item" aria-current="page"><a href="<?= BASEURL ?>produk">Katalog Produk</a></li>
          ...
          ?>
            <li class="list-group-item d-flex justify-content-between align-items-start">
              <div class="ms-2 me-auto">
                <a href="<?= BASEURL ?>produk/kategori/<?= urlencode($item['nama_kategori_produk']) ?>" class="fw-bold<?= ($data[3] == ucwords($item['nama_kategori_produk'])) ? ' aktif' : '' ?>"><?= ucwords($item['nama_kategori_produk']) ?></a>
              </div>
            </li>
          <?php
          endforeach;
    ...
    <div class="d-flex justify-content-center py-5">
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          <li></li>
          <li class="page-item">
            <a class="page-link" style="padding-right: 17px;" href="<?= BASEURL ?>produk/kategori/<?= $data[3] ?>/<?= ($data[5] - 1 <= 0) ? '1' : $data[5] - 1 ?>">
          ...
          for ($i = 0; $i < $data[4]; $i++) :
          ?>
            <li class="page-item<?= $i + 1 == $data[5] ? ' active' : '' ?>"><a class="page-link" href="<?= BASEURL ?>produk/kategori/<?= $data[3] ?>/<?= $i + 1 ?>"><?= $i + 1 ?></a></li>
          <?php endfor; ?>
          <li class="page-item"><a class="page-link" style="padding-left: 17px ;" href="<?= BASEURL ?>produk/kategori/<?= $data[3] ?>/<?= ($data[5] + 1 > $data[4]) ? $data[4] : $data[5] + 1 ?>">
