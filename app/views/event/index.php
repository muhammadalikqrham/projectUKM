      ...
      <div class="card-body">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active" aria-current="page"><b><?= $data[0]['nama_event'] ?></b></li>
          </ol>
        </nav>
      </div>
    </div>
  ...
  <div class="row">
    <div class="col-12 col-sm-12 col-md-12 col-lg-8 col-xl-8">
      <div class="card card-tambahan shadow">
        <div class="card-body">
          <h2><?= $data[0]['nama_event'] ?></h2>
          <!-- <p class="text-muted">Samar Indah </p> -->
          <img src="<?= BASEURL ?>admin/image/event/a<?= $data[0]['gambar'] ?>" class="img-fluid py-2"><br>
  ...
  <div class="row row-cols-1 row-cols-md-3 g-4">
    <div class="owl-carousel">
      ...
      <?php foreach ($data[1] as $item) : ?>
        <a href="<?= BASEURL ?>event/page/<?= urlencode($item['nama_event']) ?>">
          <div class="card card-tambahan mb-3" style="width: 100%; min-height: 300px;">
            <img src="<?= BASEURL ?>admin/image/event/a<?= $item['gambar'] ?>" class="card-img-top" height="130" alt="...">
            <div class="card-body">
              <p class="card-text"><?= (strlen($item['nama_event']) > 83) ? substr($item['nama_event'], 0, 80) . '...' : substr($item['nama_event'], 0, 80) ?></p>
            </div>
            <div class="card-footer">
              <small class="text-muted">Event Desa Jembayan Tengah</small>
            </div>
          </div>
        </a>
      <?php endforeach; ?>
