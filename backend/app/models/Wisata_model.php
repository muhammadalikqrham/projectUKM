...
class Wisata_model
{
  ...
  public function getWisataById($id)
  {
    $this->db->query('SELECT * FROM ' . $this->table . ' INNER JOIN ' . $this->linkTable . ' 
                        USING (id_kategori_wisata) WHERE tb_wisata.id_wisata = :id GROUP BY tb_wisata.id_wisata DESC');
    $this->db->bind('id', $id);
  ...
  public function getKategoriWisataById($data)
  {
    $this->db->query('SELECT * FROM '  . $this->linkTable . ' WHERE id_kategori_wisata = :id');
  ...
  public function addKategoriWisata($data)
  {
  ...
  public function editKategoriWisata($data)
  {
    $query = 'UPDATE ' . $this->linkTable .
      ' SET nama_kategori_wisata =  :kategori WHERE id_kategori_wisata = :id';
  ...
  public function postWisata($data)
  {
    var_dump($data);
  ...
  public function editWisata($data)
  {
    // var_dump($data);
    // exit;
    ...
    $query = 'UPDATE ' . $this->table .
      ' SET nama = :nama, id_kategori_wisata = :kategori_wisata, deskripsi = :deskripsi, gambar = :gambar WHERE id_wisata = :id_wisata';

    $this->db->query($query);

    $this->db->bind('id_wisata', $data[0]['id']);
    $this->db->bind('nama', $data[0]['nama']);
    $this->db->bind('kategori_wisata', $data[0]['kategori']);
  ...
  public function hapusKategoriWisata($data)
  {
    $query = 'DELETE FROM ' . $this->linkTable . ' WHERE id_kategori_wisata = :id_wisata';

    $this->db->query($query);

    $this->db->bind('id_wisata', $data);

    $this->db->execute();

    return $this->db->rowCount();
  }
  public function hapusWisata($data)
  {
    $query = 'DELETE FROM ' . $this->table . ' WHERE id_wisata = :id_wisata';
  ...
  function upload($data)
  {
    ...
    $target_path = $destination_path . 'image\wisata\a' . basename($namaFileBaru);
