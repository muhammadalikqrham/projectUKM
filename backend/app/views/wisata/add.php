<div class="row">
  <div class="col">
    ...
    <div class="card shadow mb-4">
      ...
      <div class="card-body">
        <form action="<?= BASEURL ?>wisata/postWisata" method="POST" enctype="multipart/form-data">
          <div class="form-group">
            <label for="exampleInputEmail1">Judul Wisata</label>
            <input type="text" name="nama" class="form-control" required aria-describedby="Judul Berita" placeholder="Masukkan nama Wisata">
          </div>
          <div class="form-group">
            <label for="exampleInputPassword1">Kategori Wisata</label>
            <select name="kategori" class="form-control">
              <?php foreach ($data as $row) : ?>
                <option value="<?= $row['id_kategori_wisata'] ?>"><?= $row['nama_kategori_wisata'] ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group">
